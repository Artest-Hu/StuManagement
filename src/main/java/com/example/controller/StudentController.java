...
public class StudentController {
    ...
    public ResponseEntity<String> enroll(@PathVariable Long studentId, @PathVariable Long courseId) {
        try {
            ...
            return ResponseEntity.ok("Student ID " + studentId + " successfully enrolled course: ID " + courseId);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to enroll: " + e.getMessage());
        }
    }
}
